...
executable = "/home/tony/build-dmpbbo-Desktop-Default/src/dmp/demos/demoDmpContextual"
...
main_directory = "/home/tony/dmpbbo/data_sets/demoDmpContextual"
...
for n_dmp_contextual_step in [1, 2]:
    ...
    for i_demo in range(n_demos):
      ...
      plt.setp(lines, linestyle='-',  linewidth=4, color=(0.1,0.1,1.0), label=filename)
    
    for i_repro in range(n_repros):
      filename = "reproduced0"+str(i_repro);
      lines = plotTrajectoryFromFile(directory+"/"+filename+".txt",axs)
      plt.setp(lines, linestyle='-', linewidth=1, color=(0.5,0.1,0.1), label=filename)
